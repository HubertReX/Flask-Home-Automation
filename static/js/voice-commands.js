...
function voice_cmd(cmd) {
      var data = "";
      $.ajaxSetup({ contentType: "application/json; charset=utf-8"});
      $.getJSON($SCRIPT_ROOT + '/_voice_cmd', {
        cmd: cmd,
        lang: select_language.selectedIndex
      }, function(data) {
...
if (!('webkitSpeechRecognition' in window)) {
  ...
  recognition.onerror = function(event) {
    if (event.error == 'no-speech') {
      ...
      voice_cmd('<info_no_speech>')
      ignore_onend = true;
    }
    if (event.error == 'audio-capture') {
      start_img.src = 'img/mic.gif';
      showInfo('info_no_microphone');
      voice_cmd('<info_no_microphone>')
      ignore_onend = true;
    }
    if (event.error == 'not-allowed') {
      if (event.timeStamp - start_timestamp < 100) {
        showInfo('info_blocked');
        voice_cmd('<info_blocked>')
      } else {
        showInfo('info_denied');
        voice_cmd('<info_denied>')
      }
      ignore_onend = true;
    }
  };
...
function startButton(event) {
  ...
  final_transcript = '';
  recognition.lang = 'pl-PL'; //select_dialect.value;
  recognition.start();
  ignore_onend = false;
